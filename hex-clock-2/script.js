...
let [start, unit, digits] = [0, 1, 12];

let loadState = function () {
  if (localStorage.getItem('state') !== null) {
    [start, unit, digits] = localStorage.getItem('state').split(',').map(i => +i);
  }
...
let updateState = function () {
  ...
  localStorage.setItem('state', start + ',' + unit + ',' + digits);
}

window.onload = function () {
  loadState();
  setInterval(function () {
    let x = (Date.now() / 1000 - start) / unit;
